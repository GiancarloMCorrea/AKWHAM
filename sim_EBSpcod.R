...
out <- sfLapply(101:200, run_em2)
sfStop()


results <- lapply(list.files(outdir, full=TRUE), function(x)
  readRDS(x)) %>% bind_rows %>% group_by(rep, par) %>%
...
results <- filter(results, abs(maxgrad)<10)
...
sc <- filter(results, par %in% sc_variables)
## Make plot SC:
...
results %>% group_by(rep) %>% slice_head(n=1) %>% pull(maxgrad) %>% summary
## (t)ime (s)eries we care about
ts <- filter(results, !is.na(year) & par %in% ts_variables)
## (sc)alar quantities we care about
sc <- filter(results, is.na(year) &  grepl(paste0(paste("^",sc_variables,"$", sep=""), collapse = '|'),par))
